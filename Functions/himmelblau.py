...
# ЛЯМБДА+МЮ #########################################################################################################################
# population_lplusmu=[]
def create_pochatkovu_pop_LambdaPlusMu_Himmelblau(lambda_, mu): # лямбда+мю
    population_lplusmu = []
    for _ in range(lambda_):
        parent_x = random.uniform(-5, 5)  # початкове значення батька x (1 батько із 4)
        parent_y = random.uniform(-5, 5)  # початкове значення батька y
        parent_and_children = []
...
def create_next_pop_lambdaPlusMu_Himmelblau(old_pop, lambda_): #old_pop, old_parent_and_children, lambda_, mu
    next_pop, znach_himmelblau = [],[]; best_individ =[]
    p_and_ch_disclosed_sqBrackets = []
    for j in old_pop:
        p_and_ch_disclosed_sqBrackets.extend(j)
    #himmelblau_fitness_f(x,y):
    for each_p_and_ch in old_pop:  # old_parent_and_children
        for each_ind in each_p_and_ch:
            x = each_ind[0]
            y = each_ind[1]
            znach_himmelblau.append(himmelblau_fitness_f(x, y))

    new_znach_himmelblau = []
    for each_z in znach_himmelblau:
        new_znach_himmelblau.extend(each_z) # те ж саме, що й znach_levi13, але: [[],[],[]] -> [, , ,] (не сортовані)
    error_for_each_znach = [abs(min_himmelblau - each_znach_himmelblau) for each_znach_himmelblau in new_znach_himmelblau]
    ...
    best_inds_indexes = [i for i, el in enumerate(new_znach_himmelblau) if error_for_each_znach[i] in sorted_error_for_each_znach_best_lambda] # індекси найкращих індивідів (можуть бути і батьки, і діти)
    ...
    best_individ = min(next_pop, key=lambda each_coord: abs(himmelblau_fitness_f(each_coord[0], each_coord[1]) - min_himmelblau))
    return next_pop, best_individ # лучшие индивиды(лямбда(4) штуки) и лучший индивид: [x,y]

pop_update = create_pochatkovu_pop_LambdaPlusMu_Himmelblau(lambda_,mu) # перша популяція, для входження в цикл
...
while error_real > needed_error:  #error_real > e_himmelblau_styblinskyTang_fitness_f
    next_popul = []
    next_popul_best_lambda_inds, best_in = create_next_pop_lambdaPlusMu_Himmelblau(pop_update, lambda_)
    ...
    error_real = abs(himmelblau_fitness_f(best_in[0],best_in[1])-min_himmelblau)
    print(f"Ітерація: {epoch}, знайдена точка мінімуму: {best_in}, знач.функції Хімммельблау: "
          f"{himmelblau_fitness_f(best_in[0],best_in[1])}, помилка:\\"
          f"{error_real} ",'\n')
    ...
    if error_real < needed_error:
        print(f"...Знайдена точка(аргументи) глобального мінімуму: {best_in}, епоха(популяція):{epoch}, помилка: {error_real}, \\"
              f"глобальний мінімум: {himmelblau_fitness_f(best_in[0],best_in[1])}",'\n')
...
plt.title('Графік помилки для функції Хіммельблау, Лямбда+МЮ')
plt.xlabel('Епоха')
plt.ylabel('Помилка')
plt.grid(True)
plt.show()
...
# ЛЯМБДА,МЮ #########################################################################################################################
#population_onlymu=[]
def create_pochatkovu_pop_OnlyMu_Himmelblau(lambda_, mu): # лямбда+мю
    population_onlymu = []
    for _ in range(lambda_):
...
def create_next_pop_onlyMu_Himmelblau(old_pop, lambda_):
    next_pop, znach_himmelblau = [],[]; best_individ =[]
    p_and_ch_disclosed_sqBrackets = []
    for j in old_pop:
        p_and_ch_disclosed_sqBrackets.extend(j)
...
pop_update = create_pochatkovu_pop_OnlyMu_Himmelblau(lambda_,mu) # перша популяція, для входження в цикл
error_real, epoch = 100, 0
errors_history = []
epoch_history = []
...
while error_real > needed_error:  #error_real > e_himmelblau_styblinskyTang_fitness_f
    next_popul = []
    next_popul_best_lambda_inds, best_in = create_next_pop_onlyMu_Himmelblau(pop_update, lambda_)
    ...
    epoch+=1 #збільшуємо номер ітерації(популяції)
    error_real = abs(himmelblau_fitness_f(best_in[0],best_in[1])-min_himmelblau)
    print(f"Ітерація: {epoch}, знайдена точка мінімуму: {best_in}, знач.функції Хіммельблау: {himmelblau_fitness_f(best_in[0],best_in[1])}, помилка:\\"
          f"{error_real} ",'\n')
    errors_history.append(error_real)
    epoch_history.append(epoch)
    if error_real < needed_error:
